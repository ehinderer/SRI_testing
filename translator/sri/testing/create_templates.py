...
def create_kp_template(spec):
    ...
    apititle = '_'.join(spec['info']['title'].split())
    url = spec['servers'][0]['url']
    # At the moment, there's no way to tell whether this is a TRAPI or not some url cleanup
    if url.endswith('/'):
        url = url[:-1]
...
def create_template(spec):
    ...
    if spec['info']['x-translator']['component'] == 'ARA':
        # TODO: create_ara_template is expecting a list of KP titles here? Can SmartAPI provide them?
        create_ara_template(spec=spec, kp_titles=[])
...
# There should be a path that checks to see if there's already
# a test_triples for the source and only checks to see
